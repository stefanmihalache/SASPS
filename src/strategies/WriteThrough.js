...
class WriteThroughService {
  ...
  async init() {
    ...
    console.log('✓ Write-Through Service initialized');
  }

  getCacheKey(type, id) {
    return `write-through:${type}:${id}`;
  }
  ...
  getRouter() {
    ...
    router.get('/health', (req, res) => {
      res.json({ status: 'healthy', strategy: 'write-through' });
    });
    ...
    // Read operations - same as cache-aside
    ...
    // Update product - Write-Through: Write to both cache and DB simultaneously
    router.put('/products/:id', async (req, res) => {
      ...
      try {
        ...
        const [dbSuccess] = await Promise.all([
          ...
          // Get updated product to cache it
          this.db.getProductById(req.params.id).then(product => {
            if (product) {
              const cacheKey = this.getCacheKey('product', req.params.id);
              return this.cache.set(cacheKey, { ...product, ...req.body }, this.cacheTTL);
            }
            return false;
          })
        ]);
        ...
        if (dbSuccess) {
          // Invalidate list caches since they're now stale
        ...
        if (dbSuccess) {
          res.json({ 
            message: 'Product updated successfully (write-through)',
            responseTime 
          });
        } else {
          res.status(404).json({ error: 'Product not found' });
    ...
    // Create product - Write-Through pattern
    router.post('/products', async (req, res) => {
      ...
      try {
        ...
        if (success) {
          res.status(201).json({ 
            message: 'Product created successfully (write-through)',
            responseTime 
          });
        } else {
    ...
    router.delete('/products/:id', async (req, res) => {
      ...
      try {
        ...
        const [dbSuccess] = await Promise.all([
          ...
          this.cache.del(this.getCacheKey('product', req.params.id))
        ]);
        ...
        if (dbSuccess) {
          res.json({ 
            message: 'Product deleted successfully (write-through)',
            responseTime 
          });
        } else {
          res.status(404).json({ error: 'Product not found' });
...
module.exports = WriteThroughService;
